...
#define DKGL_GLSL_ES_VERSION "#version 100\n"		/* ARB_ES2_compatibility */
...
public:
		...
		, mesh3D(NULL)
	{
		...
		DKObject<DKTexture> dummyTexture = DKTexture2D::Create(1, 1, DKTexture::FormatRGB8, DKTexture::TypeUnsignedByte, d).SafeCast<DKTexture>();
...
void DKRenderer::RenderSolidSphere(const DKVector3& center, float radius, int lats, int longs, const DKMatrix4& tm, const DKColor& color, const DKBlendState& blend) const
{
	if (radius > 0 && lats > 1 && longs > 2)
	{
		...
		for (int i = 0; i < lats; ++i)
		{
			...
			{
				float lng0 = 2 * DKGL_PI * static_cast<float>(j) / longs;
				float lng1 = 2 * DKGL_PI * static_cast<float>(j + 1) / longs;
				float x0 = cos(lng0);
				float x1 = cos(lng1);
				float z0 = sin(lng0);
				float z1 = sin(lng1);
