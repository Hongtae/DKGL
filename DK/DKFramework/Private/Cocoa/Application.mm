...
DKRect Application::DisplayBounds(int displayId) const
{
	...
	if (displayId >= 0)
	{
		@autoreleasepool {
			...
			{
				NSScreen* screen = screens[displayId];
				NSRect frame = screen.frame;			// display bounds
				bounds = DKRect(frame.origin.x, frame.origin.y, frame.size.width, frame.size.height);
			}
		}
	}
	return bounds;
...
DKRect Application::ScreenContentBounds(int displayId) const
{
	...
	if (displayId >= 0)
	{
		@autoreleasepool {
			...
			{
				NSScreen* screen = screens[displayId];
				NSRect frame = screen.visibleFrame;		// content bounds
				bounds = DKRect(frame.origin.x, frame.origin.y, frame.size.width, frame.size.height);
			}
		}
	}
