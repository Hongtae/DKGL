...
// You can even share ownership with raw pointer.
//
// Example:
...
namespace DKFoundation
{
	...
	public:
		...
		static bool IsPolymorphic() {return TypeTraits::IsPolymorphic;}
		...
		template <typename R> struct IsConvertible
		{
			enum { Result = DKTypeConversionTest<T, R>::Result };
		};
		template <typename R> R* SafeCast(void)
		{
			if (IsConvertible<R>::Result)			// up casting
