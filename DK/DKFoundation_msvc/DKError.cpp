...
#error "Platform not supported!"
...
#if	defined(__APPLE__) && defined(__MACH__)
			void __attribute__((noinline)) TraceCallStack(size_t skip, size_t maxDepth, DKArray<DKError::StackFrame>& frames)
			{
				DKCriticalSection<DKSpinLock> guard(traceLock);
	...
	DKLIB_API void SetCriticalErrorHandler(DKCriticalErrorHandler* h)
	{
#if DKERROR_HANDLE_CRITICAL_ERROR
	...
	DKLIB_API void DKErrorRaiseException(const char* fn, const char* file, unsigned int ln, const char* desc)
	{
