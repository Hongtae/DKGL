	...
	public:
		...
		Index Insert(const VALUE& value, Index pos)
		{
			...
			if (pos > count)
				pos = count;
			if (pos < count)
				memmove(&data[pos+1], &data[pos], sizeof(VALUE) * (count - pos));
		...
		Index Insert(const VALUE* value, size_t s, Index pos)
		{
			...
			if (pos > count)
				pos = count;
			if (pos < count)
				memmove(&data[pos+s], &data[pos], sizeof(VALUE) * (count - pos));
			for (Index i = 0; i < s; i++)
		...
		size_t Remove(Index pos)
		{
			...
			if (pos < count)
			{
				data[pos].~VALUE();
				if (count - pos > 1)
					memmove(&data[pos], &data[pos+1], sizeof(VALUE) * (count-pos-1));
				count--;
			}
			return count;
		}
		...
		size_t Remove(Index pos, size_t c)
		{
			...
			if (pos < count)
			{
				...
				for (; i < count - pos && i < c; i++)
					data[pos+i].~VALUE();
				if (i > 0)
					memmove(&data[pos], &data[pos+i], sizeof(VALUE) * (count-pos-i));
				count -= i;
			}
			return count;
		}
